 ...
 */`,
    `private NodoAVL<T> insertarAVL(NodoAVL<T> nodo, T {0}) {`,
    `    if (nodo == null) return new NodoAVL<T>({0});`,
    `    int compara = ((Comparable)nodo.getInfo()).compareTo({0});`,
    `    if (compara < 0) {`,
    `        nodo.setIzq(this.insertarAVL(nodo.getIzq(), {0}));`,
    `    }`,
    `    else if (compara > 0) {`,
    `        nodo.setDer(this.insertarAVL(nodo.getDer(), {0}));`,
    `    }`,
    `    else {`,
    ...
    `    // Caso LL`,
    `    if (bf > 1 && ((Comparable){0}).compareTo(nodo.getIzq().getInfo()) < 0) {`,
    `        return this.rotacionDerecha(nodo);`,
    `    }`,
    `    // Caso RR`,
    `    if (bf < -1 && ((Comparable){0}).compareTo(nodo.getDer().getInfo()) > 0)`,
 ...
 */`,
    `private NodoAVL<T> eliminarAVL(NodoAVL<T> nodo, T {0}) {`,
    `    if (nodo == null) return null;`,
    `    int compara = ((Comparable)nodo.getInfo()).compareTo({0});`,
    `    if (compara < 0) nodo.setIzq(this.eliminarAVL(nodo.getIzq(), {0}));`,
    `    else if (compara > 0) nodo.setDer(this.eliminarAVL(nodo.getDer(), {0}));`,
    `    else {`,
 ...
 */`,
    `private boolean buscarAVL(NodoAVL<T> root, T {0}) {`,
    `    if (root == null) return false;`,
    `    int compara = ((Comparable)root.getInfo()).compareTo({0});`,
    `    if (compara < 0) return this.buscarAVL(root.getIzq(), {0});`,
    `    else if (compara > 0) return this.buscarAVL(root.getDer(), {0});`,
    `    else return true;`,
    `}`,
  ],
  getInOrder: [
