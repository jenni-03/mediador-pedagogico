...
//           {direcciones
//             .slice(0, step)
//             .map((direccion, index) => ({ direccion, index }))
//             .filter(({ index }) => {
//               if (structure === "pila" || structure === "cola de prioridad") {
//                 // Solo mostrar el primer elemento (índice 0)
//                 return index === 0;
//               } else if (structure === "cola") {
//                 // Solo mostrar el primer elemento (índice 0) y el último
//                 const lastIndex = direcciones.slice(0, step).length - 1;
      ...
      {isShowingMemory && (
        <div className="flex justify-start gap-4 px-4 overflow-auto scrollbar-thin scrollbar-thumb-[#D72638]/60 scrollbar-track-transparent">
          {getVisibleElements().map((direccion, index) => {
            ...
            let realIndex = index;
            if (
              (structure === "cola" || structure === "nueva_estructura") &&
              step === 2 &&
              index === 1
            ) {
