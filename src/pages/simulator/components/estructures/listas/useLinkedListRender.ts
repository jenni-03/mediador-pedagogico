...
export function useLinkedListRender(
  ...
  const linksData = useMemo<LinkData[]>(() => {
    ...
    listNodes.forEach((n) => {
      if (n.next) {
        console.log("El nodo es " + n.value);
        console.log("El nodo es " + n.next);
      ...
      if (n.prev) {
        ...
        if (
          isCircular &&
          listNodes.length > 0 &&
          n.prev === listNodes[listNodes.length - 1].id
        ) {
          links.push({
            sourceId: n.id,
            targetId: n.prev,
  ...
  useEffect(() => {
    ...
    if (config.showTailIndicator) {
      const tailId =
        listNodes.length > 0 ? listNodes[listNodes.length - 1].id : null;
  ...
  useEffect(() => {
    ...
    if (
      !listNodes ||
      !svgRef.current ||
      (!query.toAddFirst && !query.toAddLast && query.toAddAt.length !== 2) ||
      !prevNodes
    )
      return;
    ...
    if (query.toAddFirst) {
      ...
      let existingLinksData = linksData.filter(
        (link) => link.sourceId !== newHeadNode || link.targetId !== newHeadNode
      );

      // Si es una lista circular, necesitamos filtrar también el enlace circular anterior
      if (config.showCircularLinks && prevNodes.length > 0 && prevHeadNode) {
      ...
      const prevLastNode =
        prevNodes.length > 0 ? prevNodes[prevNodes.length - 1].id : null;
      ...
      if (insertionPosition === 0) {
        ...
        const existingLinksData = linksData.filter(
          (link) => link.sourceId !== newNode || link.targetId !== newNode
        );
