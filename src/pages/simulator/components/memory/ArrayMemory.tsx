...
export function ArrayMemory({
  ...
  useEffect(() => {
    ...
    memorySegment.forEach((entry) => {
      const sizeRes = consolaRef.current?.ejecutarComando(`size address ${entry.address}`);
      if (sizeRes && sizeRes[0]) {
        newSizes[entry.address] = sizeRes[1];
      }

      const typeRes = consolaRef.current?.ejecutarComando(`type address ${entry.address}`);
      ...
      entry.value.forEach((_: any, idx: number) => {
        const res = consolaRef.current?.ejecutarComando(`address of ${entry.name}_${idx}`);
            ...
            .map((entry, index) => (
              ...
              >
                ...
                <p className="text-sm font-medium text-gray-500 mb-1">
                  Tipo: <span className="text-gray-700 font-semibold">{types[entry.address] ?? "—"}</span>
                </p>
                ...
                <div className="mt-4 w-full overflow-x-auto">
                  <table className="min-w-full border border-gray-200 text-sm rounded-xl overflow-hidden">
                    <thead className="bg-red-50 text-red-700 text-xs uppercase tracking-wider">
                      <tr>
                        <th className="py-2 px-3 border-b border-red-200">Dirección</th>
                        <th className="py-2 px-3 border-b border-red-200">Índice</th>
                        <th className="py-2 px-3 border-b border-red-200">Valor</th>
                      </tr>
                    </thead>
                    <tbody>
                      {entry.value.map((val: any, idx: number) => (
                        <tr key={idx} className="even:bg-gray-50 hover:bg-red-50 transition-all">
