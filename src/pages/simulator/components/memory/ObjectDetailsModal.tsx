...
export function ObjectDetailsModal({
  ...
  const [tempValues, setTempValues] = useState(
    entry.value.map((group: any[]) => group.find((a: any) => a.key === "value")?.value ?? "")
  );
  const [feedbacks, setFeedbacks] = useState<Record<number, { success: boolean; message: string }>>({});
  ...
  const confirmChange = (index: number) => {
    ...
    if (result) {
      ...
      setFeedbacks((prev) => ({ ...prev, [index]: { success, message } }));
