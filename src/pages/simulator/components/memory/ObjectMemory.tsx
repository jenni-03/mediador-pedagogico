                  ...
                  <table className="min-w-full text-sm text-left border border-gray-300 rounded-lg overflow-hidden">
                    <thead className="bg-gray-100 text-gray-700 font-semibold">
                      <tr>
                        ...
                        <th className="px-4 py-2 border-b border-gray-300">Key</th>
                        <th className="px-4 py-2 border-b border-gray-300">Valor</th>
                      </tr>
                    </thead>
                    <tbody>
                      {entry.value.map((group: any[], i: number) => {
                        const type = group.find((attr: any) => attr.key === "type")?.value || "unknown";
                        const key = group.find((attr: any) => attr.key === "key")?.value || "";
                        const value = group.find((attr: any) => attr.key === "value")?.value ?? "";

                        return (
                          <tr key={i} className="even:bg-gray-50">
                            <td className="px-4 py-2 border-b border-gray-200 text-gray-500">{type}</td>
