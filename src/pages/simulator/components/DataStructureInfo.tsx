...
export function DataStructureInfo({
    ...
    const codeLines = memoryAddress ? memoryAddress.trim().split("\n") : [];
    ...
    if (codeLines.length > 0) {
        const interval = setInterval(() => {
            if (visibleLines.length < codeLines.length) {
                setVisibleLines((prevLines) => [
                    ...prevLines,
                    codeLines[visibleLines.length],
                ]);
            } else {
                clearInterval(interval);
                setIsLoading(false); // Finaliza la carga cuando todas las líneas se muestran
            }
        }, 700); // Velocidad de animación ajustada

        return () => clearInterval(interval);
    }
}, [visibleLines.length, codeLines]); // Dependencias actualizadas
            ...
            <div className="flex flex-row gap-4">
                            ...
                            {visibleLines.map((line, index) => (
                                <div
                                    ...
                                    className={`transition-all duration-300 ${
                                        isLoading && index === visibleLines.length - 1 
                ...
                <div className="ml-auto flex flex-col items-end h-full">
                    {info.map((infoKey: string, index: number) => (
                        <h1 key={index} className="font-medium text-right text-md">
                            {`${infoKey.toUpperCase()}: ${
                                infoKey === "Tamaño"
