...
type LocalSimulatorProps<T extends string> = Omit<
  SimulatorProps<T>,
  "error"
> & {
  error?: LooseError;
...
export function Simulator<T extends string>({
  ...
  useEffect(() => {
    ...
    const offProgress = bus.on<{ stepId: string; lineIndex: number }>(
      "step:progress",
      ({ lineIndex }) => {
        setCurrentLine((prev) => (prev === lineIndex ? prev : lineIndex));
      }
    );
  ...
  const normalizedError = useMemo(() => {
    ...
    if (!e) return null;
    const op =
      (typeof (e as any).op === "string" && (e as any).op) ||
      lastOpRef.current ||
      "unknown";
            ...
            <div className="mb-6 flex flex-col gap-6 overflow-hidden lg:flex-row">
              ...
              <div className="flex w-full flex-col lg:w-[40%]">
                ...
                <div
                  ...
                  className="overflow-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-[#D72638]/60 max-h-[450px] flex-1 rounded-xl border border-[#2E2E2E] bg-[#1F1F22] p-4"
                >
                  <PseudoCodeRunner
