...
class RAM {
  ...
  public allocate(
    ...
    if (Array.isArray(valor)) {
      ...
      if (!subTipos || !subTipos[arrayElementKey]) {
        console.error(
          `Debe especificarse el tipo de los elementos del array '${nombre}'`
        );
        return null;
      }
      ...
      valor.forEach((element, index) => {
        ...
        const elementAddress = this.allocate(
          `${nombre}[${index}]`,
          elementType,
          element
        );
      ...
      if (!subTipos) {
        console.error(
          `Debe especificarse el tipo de los atributos del objeto '${nombre}'`
        );
        return null;
      }
      ...
      Object.entries(valor).forEach(([key, attrValue]) => {
        ...
        if (!subTipos[keyPath]) {
          console.error(
            `Falta especificar el tipo del atributo '${key}' en el objeto '${nombre}'`
          );
          return;
        }
        const attrAddress = this.allocate(
          keyPath,
          subTipos[keyPath],
          attrValue,
          subTipos
  ...
  public getMemorySegment(tipo: string): Record<string, any> | null {
    return this.almacen.obtenerSegmento(tipo);
  }
}
