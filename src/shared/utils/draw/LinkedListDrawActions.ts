...
export async function animateInsertFirst(
  ...
  if (prevHeadNode) {
    ...
    if (prevCircularLinkGroup) {
      ...
      await svg
        .select<SVGGElement>(`g#link-${prevHeadNode}-${lastNode}-circular-prev`)
        .transition()
        .duration(1000)
        .style("opacity", 0)
        .remove()
    ...
    if (nextCircularLinkGroup) {
      ...
      await svg
        .select<SVGGElement>(`g#link-${lastNode}-${prevHeadNode}-circular-next`)
        .transition()
        .duration(1000)
        .style("opacity", 0)
        .remove()
...
export async function animateInsertLast(
  ...
  if (prevLastNode) {
    ...
    if (showNextCircularLink) {
      await svg
        .select<SVGGElement>(`g#link-${prevLastNode}-${headNode}-circular-next`)
        .transition()
        .duration(1000)
        .style("opacity", 0)
        .remove()
    ...
    if (prevCircularLinkGroup) {
      ...
      await svg
        .select<SVGGElement>(`g#link-${headNode}-${prevLastNode}-circular-prev`)
        .transition()
        .duration(1000)
        .style("opacity", 0)
        .remove()
  ...
  if (newHeadNode && lastNode) {
    ...
    if (prevCircularLinkGroup || nextCircularLinkGroup) {
      ...
      tempPositions.set(newHeadNode, {
        ...
        y: nodeToRemoveCurrentPos.y,
      });
      tempPositions.set(lastNode, {
        x: lastNodeInitialXPos,
        y: nodeToRemoveCurrentPos.y,
      });
    }
    ...
    if (nextCircularLinkGroup && tempPositions) {
      ...
      const initialNextCircularPath = calculateCircularLPath(
        { sourceId: lastNode, targetId: newHeadNode, type: "circular-next" },
        tempPositions,
        SVG_LINKED_LIST_VALUES.ELEMENT_WIDTH,
        SVG_LINKED_LIST_VALUES.ELEMENT_HEIGHT
      );
    ...
    if (prevCircularLinkGroup && tempPositions) {
      ...
      const initialPrevCircularPath = calculateCircularLPath(
        { sourceId: newHeadNode, targetId: lastNode, type: "circular-prev" },
        tempPositions,
        SVG_LINKED_LIST_VALUES.ELEMENT_WIDTH,
        SVG_LINKED_LIST_VALUES.ELEMENT_HEIGHT
      );
    ...
    if (showNextCircularLink) {
      svg
        .select<SVGGElement>(
          `g#link-${prevHeadNode}-${prevHeadNode}-circular-next`
        )
        .transition()
        .duration(1000)
        .style("opacity", 0)
    ...
    if (showPrevCircularLink) {
      svg
        .select<SVGGElement>(
          `g#link-${prevHeadNode}-${prevHeadNode}-circular-prev`
        )
        .transition()
        .duration(1000)
        .style("opacity", 0)
...
export async function animateRemoveLast(
  ...
  if (newLastNode && headNode) {
    ...
    if (showNextCircularLink) {
      svg
        .select<SVGGElement>(
          `g#link-${prevLastNode}-${prevLastNode}-circular-next`
        )
        .transition()
        .duration(1000)
        .style("opacity", 0)
    ...
    if (showPrevCircularLink) {
      svg
        .select<SVGGElement>(
          `g#link-${prevLastNode}-${prevLastNode}-circular-prev`
        )
        .transition()
        .duration(1000)
        .style("opacity", 0)
