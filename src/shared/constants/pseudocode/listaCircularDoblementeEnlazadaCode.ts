...
export const getListaCircularDoblementeEnlazadaCode = (): OperationCode => ({
  ...
  insertAt: [
    ...
    "  if ({1} < 0 || {1} > {2}) {",
    "    throw new RuntimeException(",
    '      "No fue posible insertar el nodo en la posición especificada: La posición " + {1} + " no existe dentro de la Lista Doble."',
    "    );",
    "  }",
    "",
  ...
  removeAt: [
    ...
    "",
    "  if ({0} < 0 || {0} >= {1}) {",
    '    throw new RuntimeException("No fue posible eliminar el nodo en la posición especificada: La posición " + {0} + " no existe dentro de la Lista.");',
    "  }",
    "",
    "  if ({0} == 0) {",
