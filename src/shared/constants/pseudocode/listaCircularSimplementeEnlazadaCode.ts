 ...
 */`,
      ...
      `    if ({1} < 0 || {1} > {2}) {`,
      `        throw new RuntimeException("No fue posible insertar el nodo en la posición específicada: La posición " + {1} + " no existe dentro de la lista circular simple.");`,
      `    }`,
      `    NodoS<T> nuevoNodo = new NodoS<>({0});`,
      `    if (this.cabeza == null) {`,
      `        this.cabeza = nuevoNodo;`,
 ...
 */`,
      ...
      `  }`,
      `  NodoS<T> nodoEliminado = this.cola;`,
      `  if (this.cabeza == this.cola) {`,
      `      this.cabeza = null;`,
      `      this.cola = null;`,
      `  } else {`,
